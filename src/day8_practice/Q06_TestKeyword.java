...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i < answers.length; i++) { // i her bir öğrenci
            int sayac=0; // her bir öğrencinin doğru vevabını sayacağı için sayaç buraya konur.
            for (int j = 0; j <answers[i].length; j++){ // her öğrencinin cevaplarına bakar
                if (answers[i][j]==keys[j]) {
                    sayac++;
                }
            }
            System.out.println(i+1+" nolu ogrencinin kac dogru cevap sayisi : "+sayac);
        }
